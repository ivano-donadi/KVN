...
int main(int argc, char **argv)
{
  ...
  if( !loadCameraParams( camera_filename, image_size, camera_matrix, dist_coeffs ) )
  {
  ...
  cam_model.setSizeScaleFactor(3);
  ...
  // Initial pose
  cv::Mat_<double> r_vec = (Mat_<double>(3,1) << 1.01, -0.3400000000000001, 3 ),
                   t_vec = (Mat_<double>(3,1) << 0.09, 0.06, 0.2799999999999994),
...
//       optimize_hdc = false, optimize_direct = false;
  ...
  const double tensor_lambda = 50.0;
...
//    }
    ...
    if( has_roi )
    {
      ...
      draw_img = background;
    cv_ext::drawCircles( draw_img, proj_pts, 1, cv::Scalar ( 0,0,255 ) );
