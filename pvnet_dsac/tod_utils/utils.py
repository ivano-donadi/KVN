...
from tod_utils import data_pb2 as pb
...
def get_keypoints(targ_pb):
  ...
  for kp_pb in targ_pb.keypoints:
    ...
    xyzs.append(
        np.array([kp_pb.x, kp_pb.y,kp_pb.z]))
  ...
  return np.array(uvds), xyzs, np.array(visible)
...
class MeshObj:
  ...
  def segmentation(self, uvds, size):
    ...
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)  # Converts to float.
