...
from lib.utils.pvnet import pvnet_data_utils, pvnet_linemod_utils, visualize_utils, pvnet_pose_utils
...
class Dataset(data.Dataset):
    ...
    def read_data_suffix(self, anno, id, suffix, height, width):
        path = self.coco.loadImgs(int(id))[0]['file_name'+suffix]
        inp = Image.open(path)
        inp = inp.resize((width, height))
        #print(anno['fps_2d'+suffix])
        #print(anno['center_2d'+suffix])
        ...
        ratio = width/1024
        pre_mul = np.array([[ratio, 0., (ratio-1)/2],[0, ratio, (ratio-1)/2],[0.,0.,1.]])
    ...
    def read_data(self, img_id, height, width):
        ann_ids = self.coco.getAnnIds(imgIds=img_id)
        anno = self.coco.loadAnns(ann_ids)[0]

        res_L = self.read_data_suffix(anno, img_id, '_L', height, width)
        res_R = self.read_data_suffix(anno, img_id, '_R', height, width)
    ...
    def __getitem__(self, index_tuple):
        ...
        res_L, res_R = self.read_data(img_id, height, width)
        ...
        mask_L = res_L['mask']
        mask_R = res_R['mask']
        kpt_2d_L = pvnet_pose_utils.project(res_L['kpt_3d'],K,res_L['pose'])
        pose_r = res_L['pose']
        pose_r[0,3] -= baseline
        kpt_2d_R = pvnet_pose_utils.project(res_L['kpt_3d'],K,pose_r)
        kpt_3d = res_L['kpt_3d'] 
