...
from lib.evaluators.custom.pvnet_parallel import StereoObj1MEvaluator as Evaluator
...
def make_evaluators(train_loader, test_loader):
    test_evaluator = Evaluator(test_loader)
    train_evaluator = Evaluator(train_loader)
...
def make_data_loaders(cfg):
    ...
    val_cfg.split = 'val'
    val_loader = make_data_loader(val_cfg, is_train=False, max_iter=cfg.ep_iter, bkg_imgs_dir=cfg.bkg_imgs_dir, json_fn='test_val.json', LR_split='WwWwW')
    print('Train')
...
def main(cfg):
    ...
    test_evaluator, _ = make_evaluators(train_loader, validation_loader)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_kp', type=int, default=9, help='Number of Keypoints [default: 9]')
    parser.add_argument('--image_width', type=int, default=320, help='Image width [default: 320]')
    parser.add_argument('--image_height', type=int, default=320, help='Image height [default: 320]')
    parser.add_argument('--data', help='Data path to images_annotations/', required=True)
