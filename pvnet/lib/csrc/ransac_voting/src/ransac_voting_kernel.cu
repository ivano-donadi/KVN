...
__global__
void error_for_hypothesis_kernel(
    float* direct,     // [tn,vn,2]
    float* coords,     // [tn,2]
    float* hypo_pts,   // [hn,vn,2]
    float* inliers,     // [hn,vn,tn]
    float thresh,
    float k,
    int tn,
    int vn,
...
)
{
    ...
    float norm1=sqrt(nx*nx+ny*ny)+1e-6;
    float norm2=sqrt(dx*dx+dy*dy)+1e-6;

    float angle_dist=(dx*nx+dy*ny)/(norm1*norm2);
    inliers[hi*vn*tn+vi*tn+ti]= 1./(1.+ expf(-2.*k*(angle_dist-thresh)));
}
...
)
{
    ...
    error_for_hypothesis_kernel<<<bdim,tdim>>>(
        direct.data_ptr<float>(),
        coords.data_ptr<float>(),
        hypo_pts.data_ptr<float>(),
        inliers.data_ptr<float>(),
        thresh,
        k,
        tn,vn,hn
    );
