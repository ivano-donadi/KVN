...
class StereoObj1MEvaluator:

    def __init__(self, dataloader):
        self.model = dataloader.dataset.kps
        print(self.model.shape)
        self.diameter = dataloader.dataset.diameter
        self.height = dataloader.dataset.height
        self.width = dataloader.dataset.width
    ...
    def evaluate(self, output, batch):
        ...
        kpt_3d = batch['kpt_3d'][0].detach().cpu().numpy()
        K = batch['K'][0].detach().cpu().numpy()
        pose_gt = batch['pose'][0].detach().cpu().numpy()
        pose_pred = pvnet_pose_utils.pnp(kpt_3d, kpt_2d, K)
        self.proj2d.append(eval_utils.projection_2d(self.model, pose_pred, pose_gt, K))
        #TODO: change to stereo-obj 1m objects (centriuge_tube)
        if obj_name in ['centrifuge_tube']:
            mean_dist, add = eval_utils.add_metric(self.model, self.diameter, pose_pred, pose_gt, syn= True)
        else:
            mean_dist, add = eval_utils.add_metric(self.model, self.diameter, pose_pred, pose_gt, syn = False)
        self.add.append(add)
    ...
    def joint_evaluate(self, output, batch):
        ...
        var_L = output['var_L'][0].detach().cpu().numpy()
        var_R = output['var_R'][0].detach().cpu().numpy()
        kpt_3d_L = batch["kpt_3d"][0].detach().cpu().numpy()
        ...
        initial_guess_L, guess_cost_L = eval_utils.initial_guess_from_variance(var_L, kpt_2d_L, kpt_3d_L, K)
        initial_guess_R, guess_cost_R = eval_utils.initial_guess_from_variance(var_R, kpt_2d_R, kpt_3d_L, K)
        if guess_cost_L < guess_cost_R:
            initial_guess = initial_guess_L
        else:
            initial_guess = initial_guess_R
        ...
        mae, cm2, cmd5, add, proj2d, pose_pred_it = eval_utils.it_pnp_metrics(self.model, self.diameter, K, baseline, self.height, self.width, kpt_2d_L, kpt_2d_R, kpt_3d_L, var_L, var_R, pose_gt_L,syn=syn, initial_guess = initial_guess, obj_name = obj_name)
