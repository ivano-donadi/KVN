...
class Inference(object):
    ...
    def __call__(self) :
      ...
      R, t = pvnet_pose_utils.iterative_pnp(K, baseline, 720, 1280, kpt_2d_L, kpt_2d_R, var_L, var_R, kpt_3d, None, False)
      ...
      if self.vis :
        ...
        self.visualize(img_id, img, kpt_2d_pred, corner_2d_pred, corner_2d_gt)

      self.id = self.id + 1
      return pose_pred
    
    def visualize( self, img_id, img, kpt_2d_pred, corner_2d_pred, corner_2d_gt):
    
      _, ax = plt.subplots(1)
      plt.title("Image id = "+str(img_id))
      ax.imshow(img)
      for kpt in kpt_2d_pred:
