...
class Recorder(object):
    ...
    def state_dict(self):
        ...
        if self.best_loss is not None:
            scalar_dict['best_loss'] = self.best_loss
            scalar_dict['best_loss_epoch'] = self.best_loss_epoch
        return scalar_dict

    def load_state_dict(self, scalar_dict):
        self.step = scalar_dict['step']
        if 'best_loss' in scalar_dict:
            self.best_loss = scalar_dict['best_loss']
            self.best_loss_epoch = scalar_dict['best_loss_epoch']
        else:
            self.best_loss = None
            self.best_loss_epoch = None
