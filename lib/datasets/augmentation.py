...
def crop_or_padding_to_fixed_size_instance(img, mask, hcoords, th, tw, overlap_ratio=0.5):
    ...
    hrmax = int(min(hmin + (1-overlap_ratio) * fh, h - th))  # h must > target_height else hrmax<0
    wrmin = int(max(wmin + overlap_ratio * fw - tw, 0))
    wrmax = int(min(wmin + (1-overlap_ratio) * fw, w - tw))  # w must > target_width else wrmax<0
    hrmax = max(hrmax, hrmin+1)
    wrmax = max(wrmax, wrmin+1)


    hbeg = 0 if hpad else np.random.randint(hrmin, hrmax)
    hend = hbeg + th
    wbeg = 0 if wpad else np.random.randint(wrmin, wrmax) 
...
def crop_or_padding_to_fixed_size_noinstance(img, mask, hcoords, th, tw, overlap_ratio):
    h, w, _ = img.shape
    hpad, wpad = th >= h, tw >= w
    ...
    # if there is no way of returning an empty mask with the current size then revert to the instance case
    if (hmin - th <= 0 and h-th <= hmax) or (wmin-tw <= 0 and w-tw <= wmax):
        return crop_or_padding_to_fixed_size_instance(img, mask, hcoords, th, tw, overlap_ratio)
    
    # ensure crop without mask
    hbeg = 0 if hpad else (np.random.randint(0, hmin - th) if hmin > th else np.random.randint(hmax, h-th))
    wbeg = 0 if wpad else (np.random.randint(0, wmin - tw) if wmin > tw else np.random.randint(wmax, w-tw))
    # if pad then [0,wend] will larger than [0,w], indexing it is safe
    hend = hbeg + th
    wend = wbeg + tw
...
def crop_or_padding_to_fixed_size(img, mask, th, tw):
    ...
    wbeg = 0 if wpad else np.random.randint(0, w - tw) 
    # if pad then [0,wend] will larger than [0,w], indexing it is safe
    hend = hbeg + th
    wend = wbeg + tw
...
def crop_resize_instance_v2(img, mask, hcoords, imheight, imwidth,
    ...
    if np.sum(mask) > 0:
        ...
        img, mask = crop_or_padding_to_fixed_size(img, mask, imheight, imwidth)
...
def _augmentation(img, mask, hcoords, height, width, cfg):
    foreground = np.sum(mask)
    if foreground > 0:
        # randomly rotate around the center of the instance
        rotate = np.random.random() < cfg.train.rotate_rate
        if rotate:
            img, mask, hcoords = rotate_instance(img, mask, hcoords, cfg.train.rotate_min_z, cfg.train.rotate_max_z)
        make_negative = np.random.random() < cfg.train.negative_prob
        cropresize = np.random.random() < cfg.train.cropresize_rate
        if make_negative:
            img, mask, hcoords = crop_or_padding_to_fixed_size_noinstance(img, mask, hcoords, height, width, cfg.train.overlap_ratio)
        elif cropresize:
            ...
            img, mask, hcoords = crop_resize_instance_v1(img, mask, hcoords, height, width, cfg.train.overlap_ratio, cfg.train.resize_ratio_min, cfg.train.resize_ratio_max)
    else:
